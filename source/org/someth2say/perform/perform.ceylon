...
shared class TestResults<Result>(
        shared String name,
        shared Result() test,
...
//    shared variable Result|Uninitialized firstExecResult = uninitialized;
    ...
    shared Float exec(Boolean warmup = false) {
...
shared
[TestResults<Anything>+] benchmark(
        tests, name="No-name", warmupRounds = 3, benchRounds = 5, scale = 1,
    ...
    for (i in 1:benchRounds) {
        if (!veryQuiet) {
            print("\n``name``:Benchmarking round ``i``/``benchRounds``  * ``iterations`` iterations");
        }
    ...
    print("\n``name``: Summary min/max/avg/rstddev (pct): ``benchRounds`` rounds * ``iterations`` iterations");
    printStats(true);
    return results;
}

shared interface Uninitialized of uninitialized {}
